...
# ### 繪製網格誤差圖的函數
def plot_grid_with_error(sorted_flow_columns: list, H: int, W: int, mse_matrix: np.ndarray, mae_matrix: np.ndarray, 
                         save_dir: str = r"C:\thesis\code\result_ddpm_perCell"):
    """
    繪製網格圖，顯示每個網格點的誤差（MSE 和 MAE），並將結果存成圖與表格。
...
def evaluate_model_per_cell(models: dict, grid_data: GridData, test_dataset: Dataset, device: str = 'cuda', 
    ...
    # 繪製網格誤差圖並保存表格
    plot_grid_with_error(grid_data.sorted_flow_columns, H, W, mse_matrix, mae_matrix, save_dir)
...
if __name__ == "__main__":
    ...
    batch_size, epochs, lr, timesteps, patience = 500, 500, 0.0005, 500, 5
    ...
    for i in range(H):
        for j in range(W):
            ...
            trained_diffusion = train_ddpm(diffusion, train_loader, val_loader, i, j, epochs=epochs, 
                                          lr=lr, device=device, patience=patience, save_dir=save_dir, checkpoint_interval=50)
