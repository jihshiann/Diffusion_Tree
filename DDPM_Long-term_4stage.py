...
def evaluate_model(
    ...
    logger.info(f"===== 開始 {current_stage_name} 模型評估 (比較 {prefix}) =====")
    current_stage_model_trained.eval()
    ...
    for model_name, (pred_t, gen_fid_list_for_model) in model_predictions_map_eval.items():
        ...
        if torch.sum(valid_for_mape_calc_mask).item() > 0:
            ...
            mape_avg_grid = torch.mean(mape_per_element_filtered_finite).item() if mape_per_element_filtered_finite.numel() > 0 else float('inf')
        else:
            mape_avg_grid = float('inf')

        smape_numerator_per_element = torch.abs(pred_t - cs_target_all_t)
        smape_denominator_per_element = (torch.abs(cs_target_all_t) + torch.abs(pred_t)) / 2.0 + epsilon
        ...
        mape_overall_numerator = torch.sum(torch.abs(cs_target_all_t - pred_t))
        mape_overall_denominator = torch.sum(torch.abs(cs_target_all_t) + epsilon) 
    ...
    if cs_model_key_viz in model_predictions_map_eval:
        ...
        if cs_model_key_viz in error_grids_all_models:
            plot_grid_with_error_long_term(
                dataset_obj_for_viz, 
                error_grids_all_models[cs_model_key_viz], config, f"{prefix}_{cs_model_key_viz}"
            )
    ...
    if previous_stage_model_eval_instance:
        ...
        if ps_model_key_viz in model_predictions_map_eval:
            ...
            if ps_pred_tensor_viz.shape[0] > 0 and cs_target_all_t.shape[0] > 0:
                visualize_predictions_long_term(
                    generated_all_denorm_t=ps_pred_tensor_viz[0:1].clone().cpu(),
                    original_all_denorm_t=cs_target_all_t[0:1].clone().cpu(),
                    config=config, sample_idx_to_plot=0,
                    prefix=f"{prefix}_{prev_stage_enum_for_viz.name}_vs_{current_stage_name}Target_sample0"
                )
                visualize_predictions_long_term(
                    generated_all_denorm_t=torch.mean(ps_pred_tensor_viz, dim=0, keepdim=True).clone().cpu(),
                    original_all_denorm_t=torch.mean(cs_target_all_t, dim=0, keepdim=True).clone().cpu(),
                    config=config, sample_idx_to_plot=None,
            ...
            if ps_model_key_viz in error_grids_all_models:
                plot_grid_with_error_long_term(
                    dataset_obj_for_viz,
                    error_grids_all_models[ps_model_key_viz], config, f"{prefix}_{prev_stage_enum_for_viz.name.lower()}"
                )
    ...
    if previous_stage_model_eval_instance:
        ...
        if cs_model_key_for_diff in error_grids_all_models and ps_model_key_for_diff in error_grids_all_models:
            ...
            if diff_cs_ps_grids:
                plot_grid_with_error_long_term(
                    dataset_obj_for_viz, 
                    diff_cs_ps_grids, config, 
                    f"{prefix}_diff_{current_stage_name}_minus_{prev_stage_enum_for_diff.name}"
                )
            
    # --- Excel 匯出 ---
    excel_rows_to_export = []
    # (Excel 匯出邏輯，參考之前版本，確保使用正確的 model_keys 和 results/error_grids_all_models 中的鍵)
    logger.info(f"{current_stage_name} 評估 ({prefix}): 開始準備 Excel 報告的詳細指標...")
    ...
    # models_to_report_in_excel 已經在上面 model_predictions_map_eval 時隱含定義了
    for model_key_excel in model_predictions_map_eval.keys(): # 使用 model_predictions_map_eval 的鍵
        ...
        excel_rows_to_export.append({'資料來源': f"--- {model_key_excel} ({prefix} vs {current_stage_name} Target) ---",
                                     ...
                                     'MAPE (AvgGrid)': '', 'SMAPE (AvgGrid)': '', 'MAPE (Overall)': '', 'SMAPE (Overall)': '', 'FID': ''})
        for flat_idx in range(num_grid_cells_eval):
        ...
        avg_row_excel = {
            '資料來源': model_key_excel, '網格座標_R': '整體平均', '網格座標_C': '', '經度': '', '緯度': '',
    ...
    if previous_stage_model_eval_instance and \
       ...
       f"{ConditionMode(current_stage_mode.value - 1).name.lower()}_model_on_{current_stage_name.lower()}_data" in results:
        ...
        excel_rows_to_export.append({'資料來源': f"--- {diff_source_label_excel} ({prefix}) ---", # ... 其他分隔欄位 ...
                                      '網格座標_R': '', '網格座標_C': '', '經度': '', '緯度': '', 'MSE': '', 'MAE': '', 
                                     'MAPE (AvgGrid)': '', 'SMAPE (AvgGrid)': '', 'MAPE (Overall)': '', 'SMAPE (Overall)': '', 'FID': ''})
        ...
        for flat_idx in range(num_grid_cells_eval):
            ...
            diff_row_data_excel = {'資料來源': diff_source_label_excel}
            for metric_key_upper_excel in ['MSE', 'MAE', 'MAPE', 'SMAPE']:
            ...
            diff_row_data_excel.update({'網格座標_R': grid_r_coord_excel, '網格座標_C': grid_c_coord_excel, '經度': lon_coord_excel, '緯度': lat_coord_excel, 
                                        'MAPE (Overall)': 'N/A', 'SMAPE (Overall)': 'N/A', 'FID': 'N/A'})
            excel_rows_to_export.append(diff_row_data_excel)

        diff_avg_row_excel = {
            '資料來源': diff_source_label_excel, '網格座標_R': '整體平均差異', '網格座標_C': '', '經度': '', '緯度': ''}
        for metric_key_lower_base_excel in ['mse', 'mae']:
             ...
             diff_avg_row_excel[metric_key_lower_base_excel.upper()] = val_cs_avg_excel - val_ps_avg_excel if not (np.isnan(val_cs_avg_excel) or np.isnan(val_ps_avg_excel)) else np.nan
        for metric_key_lower_suffix_excel in ['mape_avg_grid', 'smape_avg_grid', 'mape_overall', 'smape_overall', 'fid']:
            val_cs_avg_excel = metrics_cs_excel.get(metric_key_lower_suffix_excel, np.nan)
            val_ps_avg_excel = metrics_ps_excel.get(metric_key_lower_suffix_excel, np.nan)
            excel_col_name_diff = metric_key_lower_suffix_excel.upper().replace("AVG_GRID", "(AvgGrid)").replace("OVERALL", "(Overall)")
    ...
    if excel_rows_to_export:
        ...
        excel_column_order_final = ['資料來源', '網格座標_R', '網格座標_C', '經度', '緯度', 
                                 'MSE', 'MAE', 'MAPE (AvgGrid)', 'SMAPE (AvgGrid)', 
        ...
        current_stage_save_dir_excel = config.get(f"{current_stage_name.lower()}_model_save_dir", config.get("save_dir_stage3", config.get("save_dir"))) # 確保有回退
        ...
        try:
            ...
            logger.info(f"{current_stage_name} 評估 ({prefix}): 詳細評估指標 (包含差異) 已匯出至: {excel_final_path}")
...
if __name__ == '__main__':
    ...
    if os.path.exists(basemodel_outputs_cache_filepath):
        try:
            ...
            if all_bm_outputs_s2_np_cond_normalized.shape[0] != len(df_for_bm_output_gen):
                logger.warning("快取的 Basemodel 輸出樣本數與預期不符，將重新生成。")
                all_bm_outputs_s2_np_cond_normalized = None
            else:
    ...
    if all_bm_outputs_s2_np_cond_normalized is None:
        logger.info("生成 Basemodel 輸出作為後續階段條件...")
        ...
        logger.info(f"  生成並正規化後的 Basemodel 輸出 (用於S2條件) - MIN: {np.min(all_bm_outputs_s2_np_cond_normalized):.4f}, MAX: {np.max(all_bm_outputs_s2_np_cond_normalized):.4f}, MEAN: {np.mean(all_bm_outputs_s2_np_cond_normalized):.4f}, STD: {np.std(all_bm_outputs_s2_np_cond_normalized):.4f}")
    ...
    # ... (確保使用 s2_new_cond_stats_for_subsequent_stages 來正規化 S2 的新特徵)
   
    logger.info("生成 Stage2 模型輸出作為 Stage3 條件...")
    ...
    s2_outputs_list = []
    with torch.no_grad():
        for i in tqdm(range(0, len(df_for_s2_output_gen), pred_bs), desc="Generating S2 Outputs for S3 Cond"):
    ...
    if os.path.exists(stage3_outputs_cache_filepath):
        try:
            ...
            logger.info(f"S3 outputs for S4 loaded from cache: {all_s3_outputs_s4_np_cond_normalized.shape}")
            if all_s3_outputs_s4_np_cond_normalized.shape[0] != len(df_for_s3_output_gen):
                 all_s3_outputs_s4_np_cond_normalized=None
        except: all_s3_outputs_s4_np_cond_normalized=None

    if all_s3_outputs_s4_np_cond_normalized is None:
        logger.info("生成 Stage3 模型輸出作為 Stage4 條件...")
        ...
        final_s3_model_to_eval.eval()
        with torch.no_grad():
            for i in tqdm(range(0, len(df_for_s3_output_gen), pred_bs), desc="Generating S3 Outputs for S4 Cond"):
        ...
        logger.info(f"  生成並正規化後的 Stage3 模型輸出 (作為S4條件) - MIN: {np.min(all_s3_outputs_s4_np_cond_normalized):.4f}, MAX: {np.max(all_s3_outputs_s4_np_cond_normalized):.4f}, MEAN: {np.mean(all_s3_outputs_s4_np_cond_normalized):.4f}, STD: {np.std(all_s3_outputs_s4_np_cond_normalized):.4f}")
#%%
    ...
    if train_loader_s4_final:
        ...
        if stage4_model:
            ...
            optimizer_s4 = optim.AdamW(list(stage4_model.parameters()), lr= learning_rate_s4), weight_decay=CONFIG.get("weight_decay")
...
#%%
# --- 步驟 S4-2 & S4-3: 訓練 Stage4 模型 ---
# --- 在此之前已完成 train_loader_s4_final, val_loader_s4_final, test_loader_s4_final 的定義 ---
# 初始化 Stage4 訓練用參數
# start_epoch_s4_train = 1
# best_val_loss_s4_train = float('inf')
# early_stopping_counter_s4_train = 0
# metrics_hist_s4_train = {'train_loss': [], 'val_loss': [], 'lr': []}
# epochs_to_run_s4 = CONFIG.get("epochs", 100)
# stage4_model = None
# if train_loader_s4_final:
#     logger.info(f"===== STAGE 4: 模型訓練 ({CONFIG['stage4_model_name']}) =====")
#     # checkpoint 路徑
#     stage4_ckpt = CONFIG["stage4_checkpoint_full_path"]
#     if os.path.exists(stage4_ckpt):
#         # 1) 從檔載入模型骨架
#         chkpt_s4 = torch.load(stage4_ckpt, map_location=CONFIG["device"], weights_only=False)
#         cfg_s4 = chkpt_s4.get('config_snapshot_at_save', CONFIG)
#         unet_s4 = UNet3D(
#             cfg_s4.get("image_channels", CONFIG["image_channels"]),
#             cfg_s4.get("base_channels_unet", CONFIG["base_channels_unet"]),
#             cfg_s4.get("time_emb_dim", CONFIG["time_emb_dim"]),
#             cfg_s4.get("condition_encode_dim", CONFIG["condition_encode_dim"]),
#             dropout_rate=cfg_s4.get("unet_dropout_rate", CONFIG.get("unet_dropout_rate", 0.05))
#         ).to(CONFIG["device"])
#         stage4_model = DDPM3D(
#             unet_model=unet_s4,
#             timesteps=cfg_s4.get("timesteps", CONFIG["timesteps"]),
#             image_size=(cfg_s4.get("D", CONFIG["D"]),
#                         cfg_s4.get("H", CONFIG["H"]),
#                         cfg_s4.get("W", CONFIG["W"])),
#             image_channels=cfg_s4.get("image_channels", CONFIG["image_channels"]),
#             condition_input_channels=cfg_s4.get("condition_input_channels", CONFIG["condition_input_channels"]),
#             condition_encode_dim=cfg_s4.get("condition_encode_dim", CONFIG["condition_encode_dim"]),
#             beta_start=cfg_s4.get("beta_start", CONFIG["beta_start"]),
#             beta_end=cfg_s4.get("beta_end", CONFIG["beta_end"]),
#             device=CONFIG["device"]
#         )
#     else:
#         # 2) 從 Stage3 檢查點初始化
#         logger.info(f"未找到 Stage4 檢查點，將從 Stage3 檢查點初始化 Stage4 模型。")
#         stage4_model = create_next_stage_model_from_previous_checkpoint(
#             config_for_current_stage_and_global=CONFIG,
#             device=CONFIG["device"],
#             current_stage_mode=ConditionMode.STAGE4
#         )

#     # 3) 建 optimizer/scheduler
#     optimizer_s4 = optim.AdamW(stage4_model.parameters(),
#                                 lr=CONFIG.get("lr",1e-4),
#                                 weight_decay=CONFIG.get("weight_decay",1e-5))
#     scheduler_s4 = ReduceLROnPlateau(optimizer_s4,
#                                     mode='min',
#                                     factor=CONFIG.get("lr_scheduler_factor",0.5),
#                                     patience=CONFIG.get("lr_scheduler_patience",3),
#                                     min_lr=CONFIG.get("lr_scheduler_min_lr",1e-7))
#     # 4) 執行 training loop (與 S3 相同架構，將 ConditionMode.STAGE3 改為 ConditionMode.STAGE4，並用 train_loader_s4_final)
#     for epoch in range(start_epoch_s4_train, epochs_to_run_s4 + 1):
#         stage4_model.train()
#         total_train_loss_epoch_s4 = 0.0

#         train_pbar_s4_loop = tqdm(train_loader_s4_final,
#                                     desc=f"Epoch {epoch} [S4 Train]",
#                                     leave=False, position=1, dynamic_ncols=True, unit="batch")
        
#         # Stage4Dataset __getitem__ returns 9 items
#         for target_s4_b, s3_out_grid_b, s4_new_feat_grid_b, *rest in train_pbar_s4_loop:

#             optimizer_s4.zero_grad()
#             target_s4_b = target_s4_b.to(CONFIG["device"])
#             s3_out_grid_b = s3_out_grid_b.to(CONFIG["device"]) # Cond1 for S4
#             s4_new_feat_grid_b = s4_new_feat_grid_b.to(CONFIG["device"]) # Cond2 for S4

#             t_s4_b = torch.randint(0, stage4_model.timesteps, (target_s4_b.shape[0],), device=CONFIG["device"]).long()
#             condition_arguments_for_s4_loss = {
#                 "stage3_output_grid_batch_for_s4": s3_out_grid_b,
#                 "stage4_new_condition_feature_grid_batch": s4_new_feat_grid_b
#             }
#             loss_s4_batch = stage4_model.p_losses(
#                 x_start_target_flow=target_s4_b,
#                 t=t_s4_b,
#                 mode=ConditionMode.STAGE4,
#                 condition_args=condition_arguments_for_s4_loss
#             )
#             loss_s4_batch.backward()
#             optimizer_s4.step()
#             total_train_loss_epoch_s4 += loss_s4_batch.item()
...
#         avg_train_loss_epoch_s4 = total_train_loss_epoch_s4 / len(train_loader_s4_final) if len(train_loader_s4_final) > 0 else 0.0
#         metrics_hist_s4_train['train_loss'].append(avg_train_loss_epoch_s4)
#         current_lr_epoch_s4 = optimizer_s4.param_groups[0]['lr']
#         metrics_hist_s4_train['lr'].append(current_lr_epoch_s4)

#         # --- Stage4 驗證邏輯 ---
#         avg_val_loss_s4_to_record = float('inf')
#         val_calculated_this_epoch_s4 = False
#         val_freq_s4 = CONFIG.get("val_calculation_freq", 4)

#         should_validate_this_epoch_s4 = False
#         if val_loader_s4_final and hasattr(val_loader_s4_final, 'dataset') and len(val_loader_s4_final.dataset) > 0:
#             if epoch == epochs_to_run_s4: # 最後一個 epoch 必須驗證
#                 should_validate_this_epoch_s4 = True
#             elif epoch >= start_epoch_s4_train and (epoch - start_epoch_s4_train +1) % val_freq_s4 == 0 : # 確保第一個 epoch 也可能驗證
#                     should_validate_this_epoch_s4 = True


#         if should_validate_this_epoch_s4:
#             val_calculated_this_epoch_s4 = True
#             stage4_model.eval()
#             total_val_loss_p_s4_epoch = 0.0
#             num_val_samples_p_s4_epoch = 0
#             actual_avg_val_loss_this_epoch_s4 = float('inf')

#             val_pbar_s4_loop = tqdm(val_loader_s4_final,
#                                     desc=f"Epoch {epoch} [S4 Validate]",
#                                     leave=False, position=1, dynamic_ncols=True, unit="batch")
#             with torch.no_grad():
#                 for target_s4_val_norm, s3_out_val_cond, s4_new_feat_val_cond, \
#                     _, _, _, _, _, _ in val_pbar_s4_loop:
                    
#                     target_s4_val_norm = target_s4_val_norm.to(CONFIG["device"])
#                     s3_out_val_cond = s3_out_val_cond.to(CONFIG["device"])
#                     s4_new_feat_val_cond = s4_new_feat_val_cond.to(CONFIG["device"])

#                     condition_args_for_s4_validation = {
#                         "stage3_output_grid_batch_for_s4": s3_out_val_cond,
#                         "stage4_new_condition_feature_grid_batch": s4_new_feat_val_cond
#                     }
#                     s4_generated_val_norm = stage4_model.sample(
#                         batch_size=target_s4_val_norm.shape[0],
#                         mode=ConditionMode.STAGE4, 
#                         condition_args=condition_args_for_s4_validation
#                     )
                    
#                     # 使用 train_dataset_s4 (Stage4 訓練數據集) 的 norm_stats_current_stage_target (Stage4 目標的統計量) 進行反正規化
#                     s4_target_stats_for_denorm = train_dataset_s4.norm_stats_current_stage_target # 假設 train_dataset_s4 已在此範圍內定義
#                     val_target_mean_s4 = s4_target_stats_for_denorm['mean']
#                     val_target_std_s4 = s4_target_stats_for_denorm['std']
#                     if val_target_std_s4 < 1e-6: val_target_std_s4 = 1.0

#                     s4_generated_val_denorm = s4_generated_val_norm * val_target_std_s4 + val_target_mean_s4
#                     s4_target_val_denorm = target_s4_val_norm * val_target_std_s4 + val_target_mean_s4
#                     s4_generated_val_denorm = torch.clamp(s4_generated_val_denorm, min=0.0)

#                     val_loss_b_s4 = F.mse_loss(s4_generated_val_denorm, s4_target_val_denorm).item()

#                     if not np.isnan(val_loss_b_s4):
#                         total_val_loss_p_s4_epoch += val_loss_b_s4 * target_s4_val_norm.shape[0]
#                         num_val_samples_p_s4_epoch += target_s4_val_norm.shape[0]
#                     val_pbar_s4_loop.set_postfix({"Val Batch MSE": f"{val_loss_b_s4:.5f}" if not np.isnan(val_loss_b_s4) else "NaN"})
            
#             if num_val_samples_p_s4_epoch > 0:
#                 actual_avg_val_loss_this_epoch_s4 = total_val_loss_p_s4_epoch / num_val_samples_p_s4_epoch
#             else:
#                 logger.warning(f"Stage4 Epoch {epoch}): 驗證時處理的樣本數為0。")
#                 actual_avg_val_loss_this_epoch_s4 = float('inf')
            
#             avg_val_loss_s4_to_record = actual_avg_val_loss_this_epoch_s4

#             if actual_avg_val_loss_this_epoch_s4 != float('inf'):
#                 scheduler_s4.step(actual_avg_val_loss_this_epoch_s4)
#                 if actual_avg_val_loss_this_epoch_s4 < best_val_loss_s4_train:
#                     best_val_loss_s4_train = actual_avg_val_loss_this_epoch_s4
#                     early_stopping_counter_s4_train = 0
#                     tqdm.write(f"Epoch {epoch}: 新最佳 Stage4 模型已儲存 (Val Loss: {best_val_loss_s4_train:.5f})。")
...
#                         'optimizer_state_dict': optimizer_s4.state_dict(),
#                         'scheduler_state_dict': scheduler_s4.state_dict(),
#                         'best_val_loss_s4': best_val_loss_s4_train,
#                         'config_snapshot_at_save': CONFIG,
#                         'metrics_hist_s4': metrics_hist_s4_train,
#                         'early_stopping_counter_s4': early_stopping_counter_s4_train,
#                         # 若有 S4 統計則存入
#                     }, stage4_ckpt)
#                 else: # 驗證損失沒有改善
#                     early_stopping_counter_s4_train += 1
#         else: # 本 epoch 不執行驗證 (或驗證 loader 為空)
#             avg_val_loss_s4_to_record = metrics_hist_s4_train['val_loss'][-1] if metrics_hist_s4_train['val_loss'] else float('inf')

#         metrics_hist_s4_train['val_loss'].append(avg_val_loss_s4_to_record)
#         val_loss_display_s4 = f"{avg_val_loss_s4_to_record:.5f}" if avg_val_loss_s4_to_record != float('inf') else "N/A"
#         if val_calculated_this_epoch_s4 and avg_val_loss_s4_to_record != float('inf'):
#             val_loss_display_s4 += " (Calc)"

#         epoch_pbar_s4.set_postfix_str(f"Tr_Loss: {avg_train_loss_epoch_s4:.4f}, Val_Loss: {val_loss_display_s4}, LR: {current_lr_epoch_s4:.1e}, ES: {early_stopping_counter_s4_train}/{CONFIG.get('early_stopping_patience', 6)}")

#         if early_stopping_counter_s4_train >= CONFIG.get("early_stopping_patience", 6):
#             tqdm.write(f"Stage4 訓練因早停機制觸發於 Epoch {epoch}。")
#             break
...
#     logger.info(f"Stage4 模型 '{CONFIG['stage4_model_name']}' 訓練完成。")
#     # Log final stats
#     final_train_loss_s4 = metrics_hist_s4_train['train_loss'][-1] if metrics_hist_s4_train['train_loss'] else float('nan')
#     final_val_loss_s4 = metrics_hist_s4_train['val_loss'][-1] if metrics_hist_s4_train['val_loss'] else float('nan')
#     logger.info(f"最終訓練統計 (S4): Train Loss: {final_train_loss_s4:.5f}, Last Val Loss: {final_val_loss_s4:.5f}")
# else:
#     logger.info("跳過 Stage4 訓練，train_loader_s4_final 未定義。")
# #%%
# # 4. 最終評估：從 Stage3、Stage4
# if stage4_model and final_s3_model_to_eval and test_loader_s4_final:
#     # 先載入 Inception V3 (同 Stage3)
#     inception_model_for_fid_final_eval = inception_v3(weights=Inception_V3_Weights.DEFAULT).to(CONFIG["device"])
#     logger.info(f"Inception V3 模型載入完成。")
    
#     logger.info("開始 Stage4 最終評估...")
#     final_s4_metrics, final_s4_errors = evaluate_model(
...
#         previous_stage_target_norm_stats=stage3_target_norm_stats_for_eval,
#         max_samples_for_fid=CONFIG.get("fid_num_samples"),
#         prefix="final_s4_evaluation"
#     )
#     logger.info(f"Stage4 最終評估指標: {json.dumps(final_s4_metrics, indent=2)}")
#     # 儲存指標
#     metrics_save_path = os.path.join(CONFIG["stage4_model_save_dir"], "final_stage4_evaluation_metrics.json")
#     with open(metrics_save_path, 'w') as f:
#         json.dump(final_s4_metrics, f, indent=4)
#     logger.info(f"Stage4 最終評估指標已儲存到: {metrics_save_path}")

# else:
#     logger.warning("跳過 Stage4 最終評估，缺少模型或資料。")
